...
class NetworkDB:
    ...
    def find_by_network_name(network_name):
        # 根据 network_name 查询网络，不进行数据转换
        return db.networks.find_one({"network_name": network_name})
    ...
    def create_full_network(user_id, network_data):
        ...
        network_data["created_at"] = datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ")
        network_data["updated_at"] = datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ")
...
class EquipmentLibraryDB:
    ...
    def add_equipment(library_id, category, equipment):
        ...
        db.equipment_libraries.update_one(
            {"_id": ObjectId(library_id)},
            {"$set": {"equipments": library['equipments'], "updated_at": datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ")}}
        )
        return True
    ...
    def create_library(library_data, user_id):
        ...
        library_data["created_at"] = datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ")
        library_data["updated_at"] = datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ")
